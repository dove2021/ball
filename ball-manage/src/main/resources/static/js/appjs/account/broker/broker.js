...
function reLoad() {
	$('#accountBrokerTable').bootstrapTable('refresh');
}
...
function freeze(){
	var rows = $('#accountBrokerTable').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
	if (rows.length == 0) {
		layer.msg("请选择要冻结的数据");
		return;
	}
...
$("#accountBrokerTable").on('dbl-click-row.bs.table', function(e, arg1){
	detail(arg1.id);
});