...
public class AccountTransferBiz {
	 ...
	 * 客户提现
	 */
	public boolean customerWithdraw(Integer customerId, BigDecimal amount, BigDecimal fee,String transcationCode) throws BizException{
		CustomerAccount customerAccount = customerAccountBiz.getCustomerAccountByCustomerId(customerId);
		...
		if (BigDecimal.ZERO.compareTo(fee) < 0){
			CustomerAccountFlow flow = new CustomerAccountFlow();
			flow.setAccountId(customerAccount.getId());
			flow.setAmount(fee);
			flow.setTransactionCode(transcationCode);
			flow.setTradeTime(new Date());
			flow.setTradeType(CustomerAccountFlowConst.TRADE_TYPE_WITHDRAW_FEE_OUT);
			try {
				if (!baseAccountTransferBiz.customerCashAccountChange(flow)){
					throw new BizException("客户资金账户变更失败");
				}
			} catch (BizException e) {
				logger.error(e.getMessage(), e);
			...
			tradeAccountFlow.setAmount(fee);
			tradeAccountFlow.setTransactionCode(transcationCode);
			tradeAccountFlow.setTradeTime(new Date());
			tradeAccountFlow.setTradeType(TradeAccountFlowConst.TRADE_TYPE_CUSTOMER_WITHDRAW_FEE_IN);
			try {
	 ...
	 * @throws BizException 
	 */
	public void customerWithdrawBack(Integer customerId, BigDecimal amount, BigDecimal fee) throws BizException{
		...
		customerAccountFlow.setTransactionCode(transcationCode);
		customerAccountFlow.setTradeTime(new Date());
		customerAccountFlow.setTradeType(CustomerAccountFlowConst.TRADE_TYPE_WITHDRAW_IN);
		try {
			if (!baseAccountTransferBiz.customerCashAccountChange(customerAccountFlow)){
				throw new BizException("客户资金账户变更失败");
			}
		...
		if (BigDecimal.ZERO.compareTo(fee) < 0){
			...
			flow.setTransactionCode(transcationCode);
			flow.setTradeTime(new Date());
			flow.setTradeType(CustomerAccountFlowConst.TRADE_TYPE_WITHDRAW_FEE_IN);
			try {
				if (!baseAccountTransferBiz.customerCashAccountChange(flow)){
					throw new BizException("客户资金账户变更失败");
				}
			...
			tradeAccountFlow.setTransactionCode(transcationCode);
			tradeAccountFlow.setTradeTime(new Date());
			tradeAccountFlow.setTradeType(TradeAccountFlowConst.TRADE_TYPE_CUSTOMER_WITHDRAW_FEE_OUT);
			try {
				baseAccountTransferBiz.tradeCashAccountChange(tradeAccountFlow);
			} catch (BizException e) {
				logger.error(e.getMessage(), e);
